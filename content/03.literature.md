...
The encouraged practices are plenty, however they vary a lot and the code produced by bioinformaticians are not up to mainstream software standards.
...
We selected articles which would be the entry point for bioinformatician who aim to improve their programming skills (Supplementary Methods).
...
The first impression Table @tbl:sq-recommendations might give is being intimidating due to the sheer amount of recommendations.
...
This muddled transmission prompts us to re-think our strategies and methods to realise the effective adoption of these software engineering notions.
...
The authors argue that selection can be done via the priorization of software quality attributes [@{https://iso25000.com/index.php/en/iso-25000-standards/iso-25010?limit=3%20}].
...
Through implicit prioritization bioinformaticians develop all software as a prorotype, even when the goal is to create a long-term product.
Therefore, we decided to set three target quality attributes as our learning goals that we have neglected in the past: reliability, performance, and extensibility (Figure @fig:climbing-figure).
...
In our sessions, we covered unit testing and discussed verification for scientific software (Figure @fig:climbing-figure).
...
To address this question, we built a shared understanding of functions and modularization (Figure @fig:climbing-figure), and expect the members of our code reviews to organize their code into modules.
...
Similarly to modularization, we first learnt about version control and container solutions (Figure @fig:climbing-figure), so that we can expect members to follow these practices.
...
We therefore established a learning club called software quality seminars, regular code reviews, and a resource sharing platform to foster team effort (Figure @fig:climbing-figure).
