...
We describe our experience and recommendations for implementing these pillars. <!-- This is a bit redundant with the last sentence of the previous section and as these two sentences are so close, it doesn't flow that well. An idea would be to remove the details "such as weekly ... seminars" from the previous sentence."
...
This expectation is largely self-inflicted as each person feels pressure to not expose their weaknesses - even within a friendly environment. <!-- the pressure is to do a good job, so to not expose weaknesses" -->
...
While code review sessions generally focus on implementation details, they can trigger discussion on any aspect of the code, including architectural considerations, documentation, and user interface design. <!-- this to me is a more logical order -->
...
To address this redundancy, we recommend implementing a system to share resources, which we describe below.

### Resource sharing ###

Resource sharing fundamentally involves ensuring that valuable resources are readily accessible to all participants.
We efficiently implemented sharing of two types of resources: external open-access resources (forums, repositories, packages, and libraries) and internal resources (including tools developed within the groups). <!-- "sharing through perspectives" sounds strange, so I edited this -->
The internal aspect is crucial as it fosters team contributions that enhance individual project development.
For instance, consider a shared repository containing various computational tools developed by various group members.
...
As a concrete example, many regular attendees of our sessions extensively implemented various software development methods we discussed in our meetings (such as object-oriented programming, user stories for documenting requirements and assumptions, using Jira for feature additions and bug reporting, and continuous integration with Git) while collaboratively working on the same codebase for the latest JASPAR database release [@doi:10.1093/nar/gkad1059]. <!-- minor: what exactly does the "while" refer to here? If these methods were implemented because all were working on the same codebase, I'd recommend shifting this part of the sentence up so that this becomes clearer. If not, it may be better to separate this into 2 sentences and put the text that is now inside () without () -->
...
<!-- " when we envision the edge cases and discuss the properties of the biological question" I do not understand this sentence, it would be best to edit it to clarify what message is being conveyed here. Part of this is e.g. that some concepts are not clearly defined. E.g. "the biological question" is vague, is this "the biological question the software can help solve" or is it "discuss scientific questions in biology" in a more general sense -->
...
Finally, the shared knowledge base and standards helps make new group members adopt good coding practices more quickly. <!-- "to make new group members" sounds a bit dictatorially to me -->
